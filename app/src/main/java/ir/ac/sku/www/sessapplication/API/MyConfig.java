package ir.ac.sku.www.sessapplication.API;

public class MyConfig {
    public static final String LOGIN_INFORMATION           = "http://app.sku.ac.ir/api/v1/user/login";
    public static final String SEND_INFORMATION            = "http://app.sku.ac.ir/api/v1/user/login";
    public static final String USER_IMAGE                  = "http://app.sku.ac.ir/api/v1/user/image";
    public static final String SFX_WEEKLY_LIST             = "http://app.sku.ac.ir/api/v1/user/sfx/zheton/list";
    public static final String SFX_MEAL_DETAIL             = "http://app.sku.ac.ir/api/v1/user/sfx/zheton/mealFoods";
    public static final String SFX_BUY_MEAL                = "http://app.sku.ac.ir/api/v1/user/sfx/zheton";
    public static final String SFX_DELETE_MEAL             = "http://app.sku.ac.ir/api/v1/user/sfx/zheton";
    public static final String SFX_INCREASE_CREDIT         = "http://app.sku.ac.ir/api/v1/user/sfx/increaseCredit/detail";
    public static final String SFX_INCREASE_CREDIT_AMOUNT  = "http://app.sku.ac.ir/api/v1/user/sfx/increaseCredit";
    public static final String MSG_MESSAGES                = "http://app.sku.ac.ir/api/v1/user/messages";
    public static final String MSG_SEND_MESSAGES           = "http://app.sku.ac.ir/api/v1/user/messages/sendMessage";
    public static final String MSG_GET_INFO_FOR_SEND       = "http://app.sku.ac.ir/api/v1/user/messages/sendMessage";
    public static final String STUDENT_REQUEST             = "http://app.sku.ac.ir/api/v1/user/studentRequests";
    public static final String STUDENT_COMPETITIONS        = "http://app.sku.ac.ir/api/v1/competitions";
    public static final String STUDENT_JOURNALS            = "http://app.sku.ac.ir/api/v1/journals";
    public static final String STUDENT_NEWS                = "http://app.sku.ac.ir/api/v1/news";
}
